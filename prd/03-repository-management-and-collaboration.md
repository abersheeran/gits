...
- `web/src/pages/repository-branches-page.tsx`
...
- 创建仓库默认初始化 `main`，并已支持独立页面下的默认分支切换。
- 当前仍没有分支保护、命名策略、受保护分支删除限制等治理能力。
...
- 仓库新增独立分支管理页，owner 可直接：
  - 通过右侧弹窗创建分支，避免创建表单和列表直接平铺在同一页面主体中
  - 删除非默认分支
  - 切换默认分支
- 仓库设置页不再承载分支操作，仅保留仓库基本设置与删除入口。
