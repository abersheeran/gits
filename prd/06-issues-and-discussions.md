...
- 更完整的结构化检查覆盖，例如 skipped / partial / multi-step validation，而不是只覆盖第一版 tests / build / lint
...
- 当前验证摘要已开始优先消费 agent 主动产出的结构化检查结果，并在缺失时回退到输出规则归纳。
- Issue 事件模型仍然偏粗。

下一步优先级：

1. 继续增强“当前在等谁”的自动状态回流。
2. 把 Issue 中的结构化验证结果继续扩展到更完整的检查覆盖和更稳定的重点输出提炼。
