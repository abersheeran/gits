...
现在 artifact 和 usage 已经能回流到 PR 页面，并开始回流到 Issue 的关联 PR 视图，但还缺少更结构化的评审输出：
...
- Session 已开始回流到 Issue / PR 主界面，PR 和 Issue 的关联 PR 视图都已有第一版验证摘要层。
- 验证结果和 artifact 仍缺少更结构化、面向评审的摘要层。

下一步优先级：

1. 把面向评审的验证摘要进一步结构化，而不是只展示轻量聚合摘要。
