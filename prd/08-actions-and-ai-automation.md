...
Actions 模块把"任务触发"转换成"可追踪执行"。默认执行路径运行在 Cloudflare 生态内，不依赖外部存储或队列服务；同时保留本地 runner 模式用于把 session 分发到用户自管执行端。
...
### 3.4.1 Local runner 握手

当仓库配置 `runner_type = local` 时，平台不进入 Queue → Container 路径，而是暴露 Worker API 供用户自管 runner 主动拉取：

- `GET /api/runner/poll`：返回当前用户可访问仓库中的最早 queued local session 及首个 queued attempt。
- `POST /api/runner/claim`：原子 claim attempt，返回临时 clone token、callback token 与 agent env；claim 成功后立即把 attempt / session 推进到 `running`。
- `POST /api/runner/callback`：本地 runner 用 JWT callback token 上报 heartbeat / completion，completion 复用 cloud callback 的 session 收敛逻辑。
- mention、workflow、interactive、dispatch、rerun 触发在创建 session/attempt 时都写入 `runner_type`；`local` 只保留 queued session 供 runner 轮询，不写入 `ACTIONS_QUEUE`。
- 仓库根目录提供 `local-runner/runner.ts` 作为独立本地执行脚本；用户使用 `GITS_TOKEN` + `GITS_PLATFORM_URL` 在本机轮询执行。
- 本地脚本复用 cloud runner 的 clone、MCP 注册、agent 命令 fallback 与 callback 结构，但将配置文件路径映射为 `$HOME/.codex/config.toml` 与 `$HOME/.claude/settings.json`，并在执行结束后恢复原文件。
- 本地脚本默认每 5 秒轮询与 heartbeat 一次，workspace 默认落在系统临时目录，也允许通过环境变量覆写；收到 `SIGINT` / `SIGTERM` 时需要中止当前执行并走 completion callback。
- completion callback 发送失败时，本地脚本至少重试 3 次，避免 session 长时间停留在运行态。
- 平台每分钟 sweep 一次本地 runner 的 stale running attempt；超过 90 秒未更新 heartbeat 视为 `heartbeat_timeout`。

本地 runner 的 `containerInstance` 固定命名为 `local-runner-{userId}`，只作为 attempt 归属与审计标识，不依赖 Durable Object keepalive。
...
| 表 | 职责 |
| --- | --- |
| `agent_sessions` | session 级状态、来源、分支、workflow、runner 类型、failure reason/stage（不含 `logs` 列） |
| `agent_session_attempts` | attempt 级生命周期（queued → running → 终态）、runner 类型、实例规格、升配来源 |
...
- 用户主动 cancel 支持 `queued` 和 `running` session；对 `running` session，平台先锁定 session/attempt 为 `cancelled`，`cloud` runner 再向对应 container 发送 `/stop`，`local` runner 仅等待下一次 heartbeat/poll 感知取消。
...
3. prompt 带入 Issue/PR/Review 上下文，并显式要求优先使用 `gits_issue_reply` / `gits_create_pull_request`，同时约束分支命名为 `fix/issue-{n}` 或 `feat/issue-{n}`
4. 执行结果以 handoff 摘要回流到 Issue / PR

### 5.2.1 Issue 自动触发 prompt

- `issue_created` / `issue_comment_added` prompt 统一输出 Issue Body、Acceptance Criteria、Conversation History、Platform Tools、Branch Naming 与 Git Push Credentials
- issue 自动触发 prompt 不再内联 Issue reply HTTP API token，平台交互默认通过注册的 MCP tools 完成
- issue / PR 相关自动触发在解析默认分支或创建目标分支时，优先消费 D1 `repository_refs` 缓存，减少对仓库 DO 在线可用性的耦合
...
- `sessions`：session workspace 占满主区域宽度，会话列表改为左侧 Sheet，用于筛选、选择、查看日志、attempt、artifact、cancel、rerun。
- `workflows`：workflow 列表 + sheet 编辑器，用于创建、编辑、启停、dispatch；仅暴露 `issue_created` / `pull_request_created` / `push`。
- `runtime`：仓库级 runner 类型、实例规格与 agent 配置覆盖；runner 类型选择器放在实例规格前，用于明确 Cloud / Local 的默认执行环境。
- session workspace 的 header 需以两行紧凑文本承载来源、更新时间、发起人、耗时和执行轮次；attempt 列表只展示有值的容器与失败原因。
- session workspace 的 badge 行需同时展示实例规格和 runner 类型，便于区分 Cloud 与 Local session。
- local 且 `running` 的 session 需在 runner badge 后展示最近 heartbeat 相对时间，优先使用 latest attempt 的 `updated_at`。
- session workspace 主视图只保留 validation summary、结构化 checks 和横向 attempts 摘要；Prompt、最新事件、日志与 artifact 内容统一移入右侧 Sheet，避免在主面板内平铺 Monaco 与长列表。
...
- `GET /api/runner/poll` 返回当前用户可访问仓库中最早的 queued local session
- `POST /api/runner/claim` 原子 claim attempt，签发 clone token 和 JWT callback token
- `POST /api/runner/callback` 本地 runner 用 JWT callback token 上报 heartbeat / completion
...
- `src/routes/api/runner-routes.ts`
- `src/routes/api/runner-routes.test.ts`
- `src/services/action-runner-config-service.ts`
- `src/routes/api/actions-routes.test.ts`
- `local-runner/runner.ts`
- `docs/MCP.zh-CN.md`
...
- 基础链路已通：poll → claim（含 running 推进）→ execute → heartbeat → completion。
- 心跳超时检测已实现：每分钟 Cron Trigger sweep，90 秒未更新视为 `heartbeat_timeout`。
- SIGINT/SIGTERM 优雅停机已实现：中止执行并走 completion callback。
- completion callback 失败时最多重试 3 次。
- 本地 runner 不复用平台 agent 配置文件，使用用户本机已有配置。
- session workspace 在 local running session 上展示最近 heartbeat 时间。
- 串行执行（一次一个 session），符合个人使用场景。
