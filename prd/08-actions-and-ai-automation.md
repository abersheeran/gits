...
- 容器内 `/run` 接收 RunRequest 后：
  ...
  - 注入环境变量和 token
  - spawn agent 进程
  - 定期向平台 Worker API 发送 heartbeat callback（附带增量日志）
...
- heartbeat 仅在校验通过后落日志并调用 Container DO `/keepalive` 续租 activity timeout
...
6. 容器运行时主动 callback heartbeat / completion → Worker 写入 attempt events（D1）+ flush 日志到 R2 + keepalive/stop DO
